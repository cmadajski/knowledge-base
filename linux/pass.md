...
The `pass` command is the standard method of handling secrets data on Unix systems. It leverages GPG (GNU Privacy Guard) for encryption. All credentials live as GPG-encrypted files in a tree structure. The default location for files is `~/.password-store`

## Installation

On Fedora/RHEL: `sudo dnf install pass`

If you don't have one already, create a new GPG key: `gpg --full-generate-key`

- select `9` for ECC (sign and encrypt)
- select `1` for Curve 25519
- select `0` for a non-expiring key (or whatever value you want)
- set your real name
- set your email address
- leave the comment empty
- select `O` to proceed
- enter your desired password twice in the popup window
